...
export const VacancyList: FC<VacancyListType> = (props) => {
  ...
  const maxPage = vacancies ? Math.ceil(vacancies.total / MAX_ITEMS_ON_THE_PAGE) : 0;
  ...
  const vacanciesForRender = isLocalPagination ? vacanciesForLocalPagination() : objects;
